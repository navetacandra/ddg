...
(async () => {
  ...
  if(regularSearch.hasNext) {
    const nextRegularSearch = await search({ query: "duckduckgo", next: regularSearch.next } /*"regular"*/);
  ...
  if(imageSearch.hasNext) {
    const imageRegularSearch = await search({ query: "duckduckgo", next: imageSearch.next }, "image");
